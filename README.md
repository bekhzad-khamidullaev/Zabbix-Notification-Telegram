...
- Авторизация пользователей по Telegram ID через Zabbix API. Бот выдаёт права в соответствии с уровнем доступа найденного пользователя.
...
|zabbix_keyboard_button_lastvalue|sting|Имя кнопки "Последние значения"|```Last value```|
|zabbix_keyboard_button_graphs|sting|Имя кнопки "Графики"|```Graphs```|
|zabbix_keyboard_button_offline|sting|Имя кнопки "Список оффлайн хостов"|```Offline```|
|zabbix_keyboard_button_problems|sting|Имя кнопки "Активные проблемы"|```Problems```|
...
### Запуск бота
Для работы инлайн-кнопок необходимо отдельно запустить скрипт `bot.py`:

```bash
python3 bot.py
```

Через него можно использовать кнопку *Offline* под уведомлением или команду
`/offline` для получения списка оффлайн хостов.
Также доступна команда `/problems` для списка текущих проблем.

## Последние значимые изменения

* Добавлены и изменены переменные в конфиг файле.
* Изменен XML.
* Добавлены опции и кнопка для получения списка активных проблем.
